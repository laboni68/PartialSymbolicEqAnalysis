...
def runRegularSE(binaryFile, functionName, functionAddress, entry, loopbound, returnType):
    ...
    elif returnType=='int64':
        x = claripy.BVS('R', 64, explicit_name=True)
        #print(len(sm.deadended))
        constraint_summary = claripy.Or(False)
        if(len(sm.deadended)>0):
            for i in range(len(sm.deadended)):
                path = sm.deadended[i]
                #print(len(path.solver.constraints))
                m_1 = path.regs.rax #for double return
                #m_1 = path.regs.eax #for int return 32 bit
                constraint_and = claripy.And(True)
                for j in range(len(path.solver.constraints)):
                    constraint_and = claripy.And(path.solver.constraints[j], constraint_and)
...
def calculateEquivalence(constraint_1, constraint_2,functionName, file, home=None):
        ...
        if home!=None:
            f=open(home+"solver"+functionName+".smt2",'w')
        else:
        ...
        if(solver.satisfiable()==False):
            ...
            file.write("==========================\n")
            print(solver._get_solver().model())
            file.write(str(solver._get_solver().model()))
            print(solver.variables)  
            file.write(str(solver.variables))
            if home!=None:
                f = open(home+"Result"+functionName+"NEQ.smt2",'w')
            else:
            ...
            file.write(str(solver1.satisfiable())+"\n") 
            if solver1.satisfiable():
                print("==========================")
                file.write("==========================\n")
                print("Not nonequivalent for all the domain space")
                file.write("Not nonequivalent for all the domain space\n")
                print("At least one equivalent solution")
                file.write("At least one equivalent solution\n")
                print(solver1._get_solver().model())
                file.write(str(solver1._get_solver().model())+"\n")
                if home!=None:
                    f1 = open(home+"Result"+functionName+"EQ.smt2",'w')
                else:
            ...
            file.write("Non equivalent for whole domain\n")
            
            

def calculatePairwiseEquivalence(constraint_1_List, constraint_2_List, file):
    countNonEquivalence=0
    countEquivalence=0
    print("constraints from binary 1 ", len(constraint_1_List))
    file.write("constraints from binary 1 "+str(len(constraint_1_List))+"\n")
    print("constraints from binary 2 ", len(constraint_2_List))
    file.write("constraints from binary 2 "+str(len(constraint_2_List))+"\n")
    for i in range(len(constraint_1_List)):
        for j in range(len(constraint_2_List)):
            ...
            if(solver.satisfiable()==False):
                ...
                file.write(str(constraint_1)+" and "+str(constraint_2)+"\n")
                print("==========================")
                file.write("==========================\n")
            else:
    ...
    file.write("Number of equivalent pair : "+str(countEquivalence)+"\n")
    print("Number of non-equivalent pair : ", countNonEquivalence)
    file.write("Number of non-equivalent pair : "+str(countNonEquivalence)+"\n")

def main(binary1, binary2, functionName, loopbound, returnType, pairwise, functionAddress, entry, pathDirected, inputBound, separateRun, file, home=None, number=None,vals=None):
    print(binary1, " ", binary2, " ", functionName, " ", functionAddress," ", entry, " ",loopbound, " ", pathDirected, " ", inputBound)
    if binary1==None or binary2==None:
        binary1="oldV"
        binary2="newV"
    if home!=None:
        binary1=home+binary1
        binary2=home+binary2
    if pathDirected=='T':
        ...
        calculateEquivalence(constraint_1, constraint_2,functionName, file, home)
    elif pairwise=='T':
        ...
        if home!=None:
            os.system("mv constraints_1.pkl "+home)
            os.system("mv constraints_2.pkl "+home)
        infile.close()
        calculateEquivalence(constraint_1, constraint_2,functionName, file, home)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--b1', metavar='binary', required=False, help='Binary file 1')
    parser.add_argument('--b2', metavar='binary', required=False, help='Binary file 2')
    ...
    parser.add_argument('--home', metavar='T', required=False, help='Location where to save')
    
    
    args = parser.parse_args()
    if args.home!=None:
        file=open(args.home+"Result.txt",'w')
    else:
    ...
    vals=args.vals, number=args.num, pairwise=args.pair, inputBound=args.ib, home=args.home, file=file)
    end_time = time.time()
    end = time.time()
